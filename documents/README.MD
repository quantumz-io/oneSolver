...
## Simulated annealing
Simulated annealing is a randomized search method for optimization problems and it is inspired by Statistical mechanics [5]. It obtains its name from the heating-cooling methodology of metal annealing. This Monte Carlo search method tries to improve a solution by walking randomly in the space of possible solutions and each move is probabilistically dependent on a parameter called "temperature". At high temperature, the random walk is unbiased and the moves are uniformly distributed over the whole space of solutions; as the temperature decreases, each move of random walk lead towards more likely solutions with a better objective value, and the distribution is more and more biased towards the optimal solution. The sequence of temperatures and lengths of time for which they are maintained is called *annealing schedule*.
...
The Exhaustive Search version of one solver 2.0 implements the following algorithm:
...
The Simulated annealing version of one solver 2.0  implements the following algorithm:
...
a single node (equivalent to parallel implementation i.e. oneSolver 1.0) to distributed exhaustive 
search on multiple nodes for graphs of size L = {25,26,27,28,29}
vertices. The performance tests were executed on the Intel(R) 
...
![AnnealingTimeOnCPU](AnnealingTimeOnCPU.svg "Fig 3")

The figure shows the performance comparison of annealing on 
a single node (equivalent to parallel implementation i.e. oneSolver 1.0) to distributed annealing on multiple nodes for graphs of size L = 512 vertices. The plot compares the annealing time versus the number of nodes with linear and geometric schedules. The performance tests were executed on the Intel(R) DevCloud cluster nodes with CPU device type: Intel(R) Xeon(R) E-2146G CPU @ 3.50GHz processor, and the Linux time utility was used to report the results.

![AnnealingTimeOnGPU](AnnealingTimeOnGPU.svg "Fig 4")

The figure shows the performance comparison of annealing on 
a single node (equivalent to parallel implementation i.e. oneSolver 1.0) to distributed annealing on multiple nodes for graphs of size L = 512 vertices. The plot compares the annealing time versus the number of nodes with linear and geometric schedules. The performance tests were executed on the Intel(R) DevCloud cluster nodes with gpu device type: Intel(R) UHD Graphics P630, and the Linux time utility was used to report the results.

![AnnealingEnergyOnCPU](AnnealingEnergyOnCPU.svg "Fig 5")

The figure shows the performance comparison of annealing on 
a single node (equivalent to parallel implementation i.e. oneSolver 1.0) to distributed annealing on multiple nodes for graphs of size L = 512 vertices. The plot compares the minimum energy versus the number of nodes for linear and geometric schedules. The performance tests were executed on the Intel(R) DevCloud cluster nodes with CPU device type: Intel(R) Xeon(R) E-2146G CPU @ 3.50GHz processor, and the Linux time utility was used to report the results.

![AnnealingEnergyOnGPU](AnnealingEnergyOnGPU.svg "Fig 6")

The figure shows the performance comparison of annealing time  on 
a single node (equivalent to parallel implementation i.e. oneSolver 1.0) to distributed annealing on multiple nodes for graphs of size L = 512 vertices. The plot compares the minimum energy versus the number of nodes for linear and geometric schedules. The performance tests were executed on the Intel(R) DevCloud cluster nodes with gpu device type: Intel(R) UHD Graphics P630, and the Linux time utility was used to report the results.
...
## Performance comparision of Parallel and distributed simulated annealing on GPU vs CPU.

![AnnealingTimeOnGPUvsCPU](AnnealingTimeOnGPUvsCPU.svg "Fig 8")

The figure shows the performance comparison of annealing algorithm on two
devices: GPU and CPU of Intel(R) DevCloud cluster for graphs of size L = 512
vertices on cluster nodes ranging from two to sixteen.

# References
1) Gary Kochenberger, Jin Kao Hao, Fred Glover, Mark Lewis, Zhipeng Lü, Haibo Wang, and Yang Wang. The unconstrained binary quadratic programming problem: A survey. Journal of Combinatorial Optimization, 28(1):58–81, 2014.
